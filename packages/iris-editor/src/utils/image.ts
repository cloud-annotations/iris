...
export async function createJPEGs(
  files: File[],
  {
    fps = 3,
    maxWidth = 1500,
  ...
  const images = files
    ...
    .map(async (file) => {
      return await imageToJPEG(file, { maxWidth, maxHeight, scaleMode });
    });

  const videos = files
    .filter(({ type }) => type.startsWith("video/"))
    .map(async (file) => {
      return await videoToJPEGs(file, { fps, maxWidth, maxHeight, scaleMode });
    });
